    ...
    value: function callRoutingProcedure(session, routerAddress) {
      return this._callAvailableRoutingProcedure(session).then(function (result) {
        ...
        if (error.code === PROCEDURE_NOT_FOUND_CODE) {
          ...
          throw (0, _error.newError)('Server at ' + routerAddress.asHostPort() + ' can\'t perform routing. Make sure you are connecting to a causal cluster', _error.SERVICE_UNAVAILABLE);
        } else {
        ...
        servers.forEach(function (server) {
          ...
          if (role === 'ROUTE') {
            routers = parseArray(addresses).map(function (address) {
              return _serverAddress2.default.fromUrl(address);
            });
          } else if (role === 'WRITE') {
