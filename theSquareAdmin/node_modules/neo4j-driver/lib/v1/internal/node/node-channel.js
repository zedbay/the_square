...
var TrustStrategy = {
  ...
  TRUST_CUSTOM_CA_SIGNED_CERTIFICATES: function TRUST_CUSTOM_CA_SIGNED_CERTIFICATES(config, onSuccess, onFailure) {
    ...
    var socket = _tls2.default.connect(config.address.port(), config.address.resolvedHost(), tlsOpts, function () {
...
function connect(config, onSuccess) {
  ...
  if (!isEncrypted(config)) {
    var socket = _net2.default.connect(config.address.port(), config.address.resolvedHost(), onSuccess);
    socket.on('error', onFailure);
    return configureSocket(socket);
