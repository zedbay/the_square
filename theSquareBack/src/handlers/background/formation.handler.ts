...
export class FormationHandler {
    ...
    public static create(req: any, res: any) {
        ...
        const request =
            `MATCH 
                (e:${claims.get('type')}), 
                (s:School { name: "${req.body.school}" }) 
            WHERE 
                ID(e) = ${v1.int(claims.get('id'))}
            CREATE 
                (f:Formation { entitled: "${req.body.entitled}", description: "${req.body.description}", endDate: "${req.body.endDate}", 
    ...
    public static get(req: any, res: any) {
        const request =
            `MATCH 
                (p:Person)-[:STUDY]->(f:Formation) 
            WHERE 
                ID(p) = ${v1.int(req.params.idPerson)} 
            RETURN f`;
    ...
    public static delete(req: any, res: any) {
        ...
        const request =
            `MATCH 
                (e:${claims.get('type')})-[s:STUDY]->(f:Formation { entitled: "${req.params.entitled}" }) 
            WHERE 
                ID(e) = ${v1.int(claims.get('id'))} 
            DETACH DELETE f`;
